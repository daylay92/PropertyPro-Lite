...
const showSideNav = () => {
  ...
  toggleBtn.onclick = () => {
    ...
    else mainOverLay.classList.add('dashboard-main__overlay--show');
  };
  mainOverLay.onclick = () => {
...
const selectNextPropertyPage = () => {
  ...
  prevBtn.onclick = e => {
    ...
    if (diff === 1) prevBtn.classList.add('pagination-dashboard__btn--disabled');
  ...
  nextBtn.onclick = e => {
    ...
    if (diff === 1) nextBtn.classList.add('pagination-dashboard__btn--disabled');
    currentPageNum.textContent++;
    e.preventDefault();
  };
};
...
const toggleStateDropdown = () => {
  ...
  const dropDown = document.querySelector('.post-prop__form-drop-down');
  const dropDownMimicWrapper = document.querySelector(
    '.post-prop__form-input-state-mimic'
  );
...
const togglePropTypeDropdown = () => {
  ...
  const dropDownMimicWrapper = document.querySelector(
    '.post-prop__form-type-input-mimic'
  );
...
const propertyTypeSelector = () => {
  ...
  const propertyTypeList = document.querySelectorAll(
    '.post-prop__form-type-drop-down-item'
  );
  ...
  const selectValue = ({ target }) => {
    ...
    else othersElem.classList.add('post-prop__form-group--hidden');
  };
